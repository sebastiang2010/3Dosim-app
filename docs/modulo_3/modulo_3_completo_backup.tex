...
Este módulo es el tercero del pipeline \textbf{3Dosim} para dosimetría hepática con Yttrium-90.
Su objetivo es procesar los resultados de la simulación Monte Carlo (archivo MCTAL de MCNP6) o, alternativamente, la convolución con un kernel de dosis precalculado, para producir \textbf{mapas de dosis absorbida 3D en Gray (Gy)}, histogramas dosis-volumen (DVH), índices radiobiológicos (BED, EUD, EQD2) y un reporte clínico en PDF.

\vspace{0.5em}
\noindent
\textbf{Pregunta que resuelve:} \textit{¿Cómo convertir los resultados crudos de MCNP (MeV/cm³/partícula) o la convolución kernel en métricas clínicamente accionables (dosis por voxel, DVH, índices de control tumoral)?}

\vspace{0.3em}
\noindent
\textbf{Resultado principal:} Mapa de dosis en Gy (NRRD), tabla DVH por ROI, índices radiobiológicos (BED, EUD, EQD2) y reporte clínico PDF.
...
El Módulo 3 ofrece dos rutas para calcular la dosis absorbida. La ruta Monte Carlo (MCTAL) procesa la salida de MCNP6, mientras que la ruta Kernel (FFT) convoluciona la actividad PET con un kernel de dosis precalculado para obtener resultados en segundos. La siguiente tabla compara ambas aproximaciones:
...
\texttt{dose\_kernel.py} & Carga de kernel.mat (sin normalizar) \\
...
La actividad administrada al paciente determina la dosis absorbida y los tiempos de simulación MCNP. La tabla siguiente presenta los rangos típicos para radioembolización hepática con $^{90}$Y y otras terapias con radionúclidos:
...
\paragraph{Nota:} La implementación en Python (\texttt{fft\_dose.py}) \textbf{no aplica}
la normalización de la línea~1. El kernel se usa tal cual del archivo \texttt{kernel.mat}
(parámetro \texttt{normalize=False} en \texttt{get\_kernel()}). Esto preserva la escala
absoluta de dosis, ya que \texttt{kernel.mat} ya incluye el factor $\tau \times 10^9$.
...
\vardef{$K$}{Kernel de dosis (sin normalizar a suma~1) [---]}
