...
\usepackage{caption}
\usepackage{booktabs}
% enumitem no disponible, usamos config nativa de enumerate
...
Sin embargo, para la convolución clínica, el kernel se normaliza a suma
unitaria:
...
Esto garantiza que la convolución conserve la actividad total:

\begin{equation}
    \sum D = \sum (S \ast K_{\text{norm}}) = \sum S
\end{equation}
