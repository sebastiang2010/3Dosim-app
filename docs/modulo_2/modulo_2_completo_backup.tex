...
\fancyhead[R]{\small Generación de Input MCNP}

% Portada
\title{\textbf{Modulo 2: Generación de Input MCNP}\\[0.5cm]
       \large Pipeline de Simulación Monte Carlo para Dosimetria Hepatica\\[0.3cm]
...
Este módulo es el segundo del pipeline \textbf{3Dosim} para dosimetría hepática con Yttrium-90.
Su objetivo es transformar la escena segmentada generada por el Módulo 1 (labelmap dosimétrica + volúmenes TC/PET + transformaciones espaciales) en un \textbf{archivo de entrada MCNP (.i)} que describe la geometría voxelizada del paciente, los materiales, la fuente radiactiva y los detectores para la simulación Monte Carlo.

\vspace{0.5em}
\noindent
\textbf{Pregunta que resuelve:} \textit{¿Cómo traducir una segmentación anatómica tridimensional a un modelo MCNP (lattice, materiales, fuente y tallies) que MCNP6 pueda ejecutar?}

\vspace{0.3em}
\noindent
\textbf{Resultado principal:} Archivo \texttt{paciente.i} (12 secciones MCNP) + archivos auxiliares (\texttt{.src}, \texttt{tissue\_config.json}) listos para MCNP6.
...
\begin{description}
    ...
    \item[CTME] Cutoff Time --- tiempo máximo de simulación.
    \item[DE4/DF4] Dose Energy / Dose Function --- conversión fluencia a dosis.
    ...
    \item[HU] Hounsfield Units --- unidades de atenuación en TC.
    ...
    \item[RLE] Run-Length Encoding (compresión de geometria).
...
% 1. VISION GENERAL
% ============================================================
\section{Visión General --- Arquitectura del Módulo 2}
\label{sec:vision}

\subsection{Contexto Clínico}

La dosimetría personalizada en terapias con radionúclidos ($^{90}$Y, $^{177}$Lu, $^{131}$I) requiere transportar partículas a través de una geometría voxelizada derivada de la TC del paciente. El Módulo 2 toma la escena segmentada generada por el Módulo 1 (labelmap + transforms + volúmenes) y produce un archivo de entrada listo para MCNP (.i) que describe:

\begin{itemize}
    \item La geometría voxelizada del paciente (lattice + RLE).
    \item La composición y densidad de cada material (6 tejidos base + 53 tejidos ICRP-110).
    \item La fuente radiactiva (isótopo, actividad, distribución espacial).
    \item Los detectores (tallies) para estimar dosis absorbida.
\end{itemize}

El archivo generado se entrega al Módulo 3 para ejecutar MCNP y procesar resultados.
...
\midrule
MCNP & Monte Carlo N-Particle (código de transporte, LANL) \\
...
\begin{enumerate}
    ...
    \item \textbf{validate\_prereqs()}: Verifica que no haya valores nulos o inconsistentes antes de pasar a generación.
    \item \textbf{generate\_mcnp(params)}: Paso central que invoca \texttt{mcnp\_generator.py} con los parámetros del tratamiento y construye las 12 secciones del archivo .i.
...
\subsection{Parametros de Configuración}
...
\subsection{Relación con Modulo 1 y Modulo 3}
...
Materiales asignados & Voxel con índice 0 (sin material) & Critico \\
Archivo .src generado & No existe o está vacio & Critico \\
\bottomrule
\end{tabular}
\end{table}
...
Cargar correctamente está escena es el paso inicial del Modulo 2 y condiciona toda la generación posterior.
...
La función \texttt{load\_scene(mrb\_path)} usa la API de 3D Slicer:
...
\texttt{labelmap\_node} & \texttt{vtkMRMLLabelMapVolumeNode} & Segmentación con materiales \\
...
La matriz de transformación IJK$\to$RAS se obtiene con:
...
Esta matriz de $4 \times 4$ permite convertir cualquier índice de voxel $(i,j,k)$ a coordenadas RAS en mm:
...
\vardef{$\mathbf{M}_{IJK \to RAS}$}{Matriz de transformación $4\times4$ de coordenadas voxel (IJK) a coordenadas mundo (RAS)}
...
Los flags \texttt{flip\_rows} y \texttt{flip\_z} del generador MCNP manejan la reorientación de ejes entre ambos sistemas.
...
\vardef{$S_x, S_y, S_z$}{Espaciado del voxel en mm en cada dirección}
\vardef{$R$}{Matriz de rotación $3\times3$ del volumen}
\vardef{$O_x, O_y, O_z$}{Origen en RAS del voxel $(0,0,0)$ en IJK}

\subsection{Ejemplo de Extracción de Parametros}
...
\begin{tabular}{lll}
\toprule
\textbf{Error} & \textbf{Causa} & \textbf{Acción} \\
\midrule
...
\end{tcolorbox}
\end{tcolorbox}
\section{Asignación de Materiales}
...
\textbf{Acronimo} & \textbf{Significado} \\
\midrule
HU & Hounsfield Units (unidades de atenuación radiologica) \\
ICRP & International Commission on Radiological Protection \\
ICRU & International Commission on Radiation Units \\
TS & TotalSegmentator \\
...
Cada voxel en el labelmap contiene un índice numérico que representa el tejido biologico en esa posición. Para que MCNP pueda transportar particulas correctamente, cada índice debe mapear a:

\begin{enumerate}
    \item Una composición elemental (fracción en peso de cada isótopo).
    \item Una densidad (en g/cm$^3$).
    \item Un rango de HU de referencia (opcional, para validación).
\end{enumerate}
...
\subsection{Indices de Tejidos Base (Phantom, índices 1--100)}
...
\subsection{Indices ICRP-110 (Completos, índices 101--153)}
...
La composición se expresa como fracción en peso de cada isótopo (Zzzzz AAAA, por ejemplo $1000 = {}^1$H, $6000 = {}^{12}$C).
...
La constante \texttt{TS\_SEGMENT\_MAP} en \texttt{mcnp\_materials.py} mapea los nombres de segmentos de TotalSegmentator a los índices del phantom:
...
\begin{tabular}{llc}
\toprule
\textbf{Condición} & \textbf{Nuevo material} & \textbf{Rango HU} \\
\midrule
...
Densidad física & $\rho \leq 0$ o $\rho \geq 20$ g/cm$^3$ & Critico \\
Composición suma $\sim 1.0$ & $|\Sigma \text{fracciones} - 1| > 10^{-4}$ & Critico \\
...
\end{tcolorbox}
\end{tcolorbox}
\section{Geometría Voxelizada MCNP}
\label{sec:geometria}
...
\vardef{$n_x$}{Numero de voxeles en dirección X}
...
Cada celda voxel única se define una sola vez. La matriz FILL del lattice asigna que celda (material) está en cada posición $(i,j,k)$.
...
El algoritmo recorre cada fila del plano k y reemplaza secuencias del mismo material por el factor de repetición:
...
\subsection{Reorientación de Ejes (flip\_rows y flip\_z)}

Los flags \texttt{flip\_rows} y \texttt{flip\_z} controlan la orientación del volumen.
...
\subsection{Precision de Cuantización}

Las coordenadas de las superficies RPP se cuantizan a 1 micra de precisión:
...
\midrule
Tamano del .i & Lineal con $n_{voxels}$ únicos (RLE reduce $\sim 10\times$) \\
Tiempo de carga MCNP & Proporcional al tamano del FILL \\
Memoria MCNP & 1--2 GB para volumenes típicos ($512^3$) \\
...
\end{tcolorbox}
\end{tcolorbox}
\section{Definición de la Fuente (Source)}
\label{sec:fuente}
...
En dosimetria de radionuclidos, la fuente de radiación está distribuida en el volumen del paciente (no es un haz externo). El $^{90}$Y, $^{177}$Lu o $^{131}$I se administra al paciente y se acumula selectivamente en tejidos. La fuente MCNP debe representar está distribución volumétrica con la energia espectral correcta.

\subsection{Espectro de Energias --- Y-90 ($^{90}$Y)}

El $^{90}$Y decae por emisión beta pura a $^{90}$Zr (estable). La energia máxima es:

$$
E_{\beta,\text{max}}^{^{90}\text{Y}} = 2.28\ \text{MeV}
$$

\vardef{$E_{\beta,\text{max}}$}{Energia máxima de la emisión beta del $^{90}$Y (2.28 MeV)}
...
La función de densidad de probabilidad (PDF) del espectro beta teorico de Fermi:

$$
\frac{dN}{dE} = C \cdot F(Z, E) \cdot p \cdot E \cdot (E_{\max} - E)^2
$$

\vardef{$C$}{Constante de normalización del espectro beta}
\vardef{$F(Z, E)$}{Función de Fermi (corrección coulombiana)}
\vardef{$p$}{Momento del electron ($p = \sqrt{E^2 - m_e^2 c^4}$)}
\vardef{$E$}{Energia cinetica del electron}
\vardef{$E_{\max}$}{Energia máxima del decaimiento beta (2.28 MeV para $^{90}$Y)}
\vardef{$Z$}{Numero atomico del núcleo hijo ($^{90}$Zr, $Z=40$)}
...
La tarjeta SDEF generada para fuente volumétrica con archivo .src externo:
...
\vardef{$WGT$}{Peso de la particula en MCNP (1.00033788800193 para normalización)}

Los parámetros clave son:

\begin{description}
    \item[PAR=D1] Tipo de particula (1 = foton, 2 = electron).
    \item[ERG=FPAR=D6] Energia desde distribución D6 (espectro).
    \item[X=D2 Y=D3 Z=D4] Posición muestreada desde distribuciones uniformes.
    \item[CELL=D5] Celda de la fuente.
    \item[WGT=1.00033788800193] Peso constante para normalización.
\end{description}
...
Para fuentes distribuidas, el generador produce un archivo donde cada línea contiene las coordenadas de emisión de una particula:
...
El peso de cada particula (WGT) representa su fracción de la actividad total:
...
\begin{tabular}{lll}
\toprule
\textbf{Parametro} & \textbf{Valor típico} & \textbf{Dependencia} \\
\midrule
...
Volumen fuente & Tumor / Higado & Segmentación \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{Verificación} & \textbf{Metrica} & \textbf{Umbral} \\
\midrule
Espectro definido para isótopo & Falta entrada en SPECTRA dict & Critico \\
Archivo .src generado (si aplica) & No existe o 0 particulas & Critico \\
SDEF tiene ERG y WGT consistentes & Mismatch entre parámetros & Critico \\
...
DE4/DF4 & Dose Energy / Dose Function (conversión fluencia $\to$ dosis) \\
...
\begin{itemize}
    ...
    \item \textbf{*F8}: detectores puntuales en órganos de interes (higado, tumor, rinones).
    \item \textbf{F6:e}: energia depositada total para verificación de conservación.
\end{itemize}

\subsection{TMESH --- Tally de Grilla 3D}

TMESH es un tally tipo 1 (fluencia) sobre una malla cartesiana. La definición en el input MCNP:
...
\begin{tabular}{lll}
\toprule
\textbf{Parametro} & \textbf{Significado} & \textbf{Valor típico} \\
\midrule
...
El tally RMESH1:PEDEP usa un par DE4/DF4 para convertir fluencia de electrones (MeV/cm$^2$) a dosis (MeV/g). Los 11 puntos de la conversión:
...
\vardef{$DE4$}{Vector de energias para conversión fluencia $\to$ dosis (MeV)}
\vardef{$DF4$}{Vector de factores de conversión (MeV/g)/(MeV/cm$^2$)}

La conversión responde a la formula general:
...
\vardef{$\mu_{en}(E)/\rho$}{Coeficiente masico de absorción de energia (cm$^2$/g)}
...
La notación \texttt{lc} permite especificar una lista de celdas que conforman el detector.

La desviación estandar relativa (RSD) para *F8:

$$
RSD = \frac{1}{\sqrt{N_{eventos}}}
$$

\vardef{$RSD$}{Desviación estandar relativa del tally *F8}
...
Controlan la energia mínima de simulación:
...
El parámetro \texttt{J} indica que es un corte en energia (no en tiempo).
...
\subsection{Parametros Globales de Simulación}
...
\vardef{$NPS$}{Numero de historias de particulas en la simulación MCNP}
\vardef{$RAND$}{Tarjeta de control de numeros aleatorios}

\subsection{Verificación de Conservación de Energía}

Antes de ejecutar MCNP se verifica que la energía total emitida por la fuente sea consistente con la actividad del paciente y el número de partículas simuladas. La energía total emitida se calcula como:
...
\end{tcolorbox}
\end{tcolorbox}
\section{Generación del Archivo .i}
...
La generación del input MCNP es el paso central del Modulo 2. Recibe los datos de la escena procesada (labelmap, TC, VOIs, parámetros del tratamiento) y produce un archivo de texto con formato MCNP6 (.i) listo para ejecutar.
...
\subsubsection{Sección 1: Cabecera}
...
\subsubsection{Sección 2: Bloque de Celdas}
...
\subsubsection{Sección 3: Bloque de Superficies}
...
\subsubsection{Sección 4: Tarjetas de Datos}
...
\subsubsection{Sección 5: Materiales}
...
\subsubsection{Sección 6: Fuente SDEF}
...
\subsubsection{Sección 7: Tallies TMESH}
...
\subsubsection{Sección 8: Tallies *F8}
...
\subsubsection{Sección 9: Tally F6}
...
\subsubsection{Sección 10: Print}
...
\subsubsection{Sección 11: Archivo de Fuente Externo}
...
\subsubsection{Sección 12: Corte de Geometria (Opcional)}

Solo se incluye si la variable de entorno \texttt{DOSIM\_DEBUG\_GEOM} está activa:
...
El formato MCNP limita cada línea a 72 caracteres. El generador respeta está restricción:
...
\textbf{Verificación 1: Tamano mínimo}

\begin{lstlisting}
assert os.path.getsize(path) > 10_000, "Archivo demasiado pequeno"
\end{lstlisting}

\textbf{Verificación 2: Keywords obligatorias}
...
\begin{tabular}{lll}
\toprule
\textbf{Archivo} & \textbf{Contenido} & \textbf{Tamano típico} \\
\midrule
...
\texttt{paciente.debug\_geom.txt} & Debug geométrico (si DEBUG) & 1--5 KB \\
\bottomrule
\end{tabular}
\end{center}
...
\end{tcolorbox}
\end{tcolorbox}
\section{Pipeline Completo --- Integración del Modulo 2}
\label{sec:pipeline}
...
\begin{tabular}{lll}
\toprule
\textbf{Elemento} & \textbf{Formato} & \textbf{Descripción} \\
\midrule
...
Espectro & .dat & Distribución de energias \\
Metadata & .json & Parametros usados en la generación \\
\bottomrule
\end{tabular}
\end{center}
...
\texttt{NodeNotFound} & Falta volume/labelmap & Verificar segmentación \\
...
\texttt{SourceSamplingError} & VOI vacio & Verificar segmentación tumor \\
\bottomrule
\end{tabular}
\end{center}
...
def run_dosimetry_from_scene(mrb_path, params):
    # ===== Modulo 1: Segmentación =====
    scene = slicer.util.loadScene(mrb_path)
    # ...

    # ===== Modulo 2: Generación de input MCNP =====
...
Tiempo de generación & $t_{gen} < 5$ min & Advertencia \\
Conservación de energia & Ratio 0.95--1.05 & Critico \\
